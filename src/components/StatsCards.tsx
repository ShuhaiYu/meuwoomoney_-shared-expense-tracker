  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="bg-white p-5 rounded-3xl shadow-sm border-2 border-transparent hover:border-cat-teal/30 transition">
        ...
        <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">Remaining Budget</p>
        ...
        {stats.felixExtraDeposits > 0 && (
          <span className="text-[11px] text-blue-500 font-bold block mt-1">+${stats.felixExtraDeposits.toFixed(2)} extra deposited</span>
        )}
        {hasActiveFilters && <span className="text-[11px] text-gray-500 block mt-1">(Filtered View)</span>}
      </div>
      ...
      <div className="bg-white p-5 rounded-3xl shadow-sm border-2 border-transparent hover:border-cat-brown/30 transition">
        ...
        {stats.sophieExtraDeposits > 0 && (
          <span className="text-[11px] text-blue-500 font-bold block mt-1">+${stats.sophieExtraDeposits.toFixed(2)} extra deposited</span>
        )}
        {hasActiveFilters && <span className="text-[11px] text-gray-500 block mt-1">(Filtered View)</span>}
      </div>
    </div>
